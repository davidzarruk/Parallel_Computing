...
nloptinst:
	wget http://ab-initio.mit.edu/nlopt/nlopt-2.4.2.tar.gz -P $(HOMET)
	tar  -zxvf $(HOMET)nlopt-2.4.2.tar.gz -C$(HOMET)
	mkdir $(HOMET)install
	cd $(HOMET)nlopt-2.4.2 && ./configure --prefix=$(HOMET)install && make && make install
